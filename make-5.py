...
with open('one_word_with_double_word.json', 'r', encoding='utf-8-sig') as f:
    ...
    def make_sentence(no, pingze_type, word_type, rhyme=rhyme):
        ...
        if pingze_type == 0:
            ...
            if word_type:
                ...
                sentence += get_pingze(double_word_with_one_word[double_word], False)
            else:
                word_4 = get_pingze(pos_data[str((no - 1) * 5 + 3)][:400])
                double_word = get_double_word(word_4, end=False, ping=False)
                word_3 = get_pingze(double_word_with_one_word[double_word])
                sentence += word_3
                sentence += double_word
        elif pingze_type == 1:
            # pingze_type 1: 平平仄仄平
            ...
            if word_type:
                ...
                word_3 = get_pingze(double_word_with_one_word[double_word], ping=False)
                sentence += word_3
                sentence += double_word
        if pingze_type == 2:
            # pingze_type 2: 平平平仄仄
